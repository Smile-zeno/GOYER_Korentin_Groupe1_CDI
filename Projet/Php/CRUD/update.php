...
try {
    if (isset($_GET['id'])) {
        $requete = $pdo->prepare("SELECT * FROM avis WHERE id = :id");
        $requete->execute([
            'id' => $_GET['id']
        ]);
    ...
    if (isset($_POST['nom']) && isset($_POST['titre']) && isset($_POST['note']) && isset($_POST['avi']) && isset($_GET['id'])) {
        $modification = $pdo->prepare("UPDATE avis SET nom = :nom, titre = :titre, avi = :avi, note = :note WHERE id = :id");
    ...
    echo "Erreur lors de la modification de l'avi : " . $e->getMessage();
}
...
<body>
    ...
    <main class="CRUD">
        <!-- CRUD -->
        <h1 class="CRUD_element">Modifier l'avi</h1>

        <form method="POST" action="update.php?id=<?php echo $avis['id']; ?>" class="CRUD_element">
            <label for="nom">Nom</label>
            <input type="text" name="nom" value="<?php echo $avis['nom']; ?>" required>
            <br>
            <label for="titre">Titre du film</label>
            <input type="text" name="titre" value="<?php echo $avis['titre']; ?>" required>
            <br>
            <label for="note">Note sur 10</label>
            <input type="number" name="note" value="<?php echo $avis['note']; ?>" max="10" required>
            <br>
            <label for="avi">Avi</label>
            <textarea name="avi" required><?php echo $avis['avi']; ?></textarea>
            <button type="submit" class="CRUD_element">Modifier</button>
        </form>
    </main>
